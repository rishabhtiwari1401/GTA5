...
startGame();// Function to animate the planets' orbits
function startAnimation() {
    const orbits = document.querySelectorAll('.orbit');
    const planets = document.querySelectorAll('.planet');
    
    // Assigning rotation to each planet's orbit
    orbits.forEach((orbit, index) => {
        let rotationSpeed = (index + 1) * 20; // Varying speed for different planets
        orbit.style.animation = `rotatePlanet ${rotationSpeed}s linear infinite`;
    });

    // Add event listeners to display planet info on click
    planets.forEach((planet) => {
        planet.addEventListener('click', () => {
            displayPlanetInfo(planet.getAttribute('data-name'));
        });
    });
}

// Function to display the clicked planet's name
function displayPlanetInfo(planetName) {
    const infoDiv = document.getElementById('planet-info');
    infoDiv.innerHTML = `You clicked on: <strong>${planetName}</strong>`;
}
